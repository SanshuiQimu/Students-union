...
def _default_versions():
    """全部历史版本数据"""
    return [
        {"version":"3.0.330.8","versionCode":3308,"date":"2026-08-22","tag":"Beta","changelog":["新增兼任模式：一人可兼任多部门","修复底部小白条沉浸：内容延伸到底部导航栏后方，同时小白条保持透明可见","修复版本检测：修正Android注入版本号参数顺序，检测更新和版本号显示恢复正常","代码变更文件：main.py：_supabase_request、_sync_members_to_supabase、_load_members_from_supabase、auth_login/auth_update/auth_register、get_members接口。index.html：_setupLoginSession、confirmDelete、directDismiss、sendTargetedMsg、getMyMessages消息过滤逻辑。supabase.sql：补充 user_table 兼任列 ALTER TABLE 脚本"],"downloadUrl":"/api/download/apk?vc=3308"},
        {"version":"3.0.330.7","versionCode":3307,"date":"2026-08-22","tag":"已撤包","changelog":["修复全屏沉浸模式：内容延伸至状态栏后方，顶部沉浸效果修复","底部导航栏小白条保持可见，不隐藏系统手势导航","状态栏与导航栏透明，刘海屏挖孔屏适配","修复APK下载失败问题","修复部门名称规范化（秘书处→办公室）"],"downloadUrl":""},
        {"version":"3.0.330.0","versionCode":3300,"date":"2026-08-22","tag":"已撤包","changelog":["新增主席团部门","新增副处长、副会长职位","全站图标统一采用IconPark图标库(iconpark.oceanengine.com/official)","校徽logo与外观设置图标除外","权限逻辑同步更新，副会长享有主席团管理权限，副处长享有办公室管理权限","干事更名为部员","秘书处更名为办公室，社联会更名为社团管理部","成员列表全部自动按部门顺序排序：主席团、办公室、宣传部、体育部、生活部、纪检部、学习部、社团管理部","点击关于本系统跳转至os3.ssqm.top","修复APK闪退问题：运行时权限请求、通知通道、错误页面、进度条、主题注入","修复了已知问题，提升系统流畅度"],"downloadUrl":""},
